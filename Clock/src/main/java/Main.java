	...
	public static void main(String[] args) {
		...
		while (true) {
	    	...
	    	if(resync >= RESYNC_INTERVAL) {
	    		...
	    		if(LocalTime.now().getHour() == alarmHour && LocalTime.now().getMinute() == alarmMinute) { playAlarm = true; (new Thread(() -> { while(playAlarm) { play(timerSoundFile); } alarmHour = -1; alarmMinute = -1; redraw(); saveFile(); } )).start(); };
